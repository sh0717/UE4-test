...
void AClock::BeginPlay()
{
    ...
    UGameplayStatics::GetAllActorsOfClass(GetWorld(), ATimeHandler::StaticClass(), TimeOfDayHandlers);
    if (TimeOfDayHandlers.Num() != 0) {
        auto Message = FString::Printf(TEXT("%oh there is timehandler") );
        GEngine->AddOnScreenDebugMessage(-1, 10, FColor::Red, Message);

        auto TimeHandler = Cast<ATimeHandler>(TimeOfDayHandlers[0]);
        MyDelegateHandle = TimeHandler->OnTimeChanged.AddUObject(this, &AClock::TimeChanged);
    }
